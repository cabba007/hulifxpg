...
namespace WebApplication5
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        {
            if (string.IsNullOrEmpty(Request.Form["pw"]) || string.IsNullOrEmpty(Request.Form["id"])) return;
            ...
            OracleCommand myCmd = new OracleCommand("SELECT DECODE(comm.f_descript(staff_dict.password),:p0,'1','0') , staff_dict.DEPT_CODE,dept_dict.dept_name,staff_dict.name from staff_dict left join dept_dict on staff_dict.dept_code = dept_dict.dept_code WHERE emp_no = :p1", myConn);
            ...
            if (myReader.Read())
            {
                if ("1" == myReader[0].ToString())
                {
                    ...
                    Session["dept_name"] = myReader[2].ToString().TrimEnd(new char[] { '护', '理', '站' });
